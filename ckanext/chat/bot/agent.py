...
front_agent_prompt = (
    ...
    "   ALSO matches existence/inventory questions about a TOPIC:\n"
    "     'haben wir etwas zu [Thema]?', 'gibt es Daten/Projekte/Dokumente über...',\n"
    "     'haben wir schon mal an/mit [Thema] gearbeitet?', 'gibt es bei uns...',\n"
    "     'kennen wir...', 'do we have anything on...', 'is there data about...'\n"
    "   These are search requests — the user wants to find content about a topic in CKAN.\n"
    ...
    "Step 3 — SYNTHESIZE (immediately — no further tool calls):\n"
    ...
    "- NEVER call literature_analyse in this workflow. The summaries are sufficient for an overview.\n"
    "  If the user wants deeper analysis of a specific document, they will ask in a follow-up.\n"
    "- Synthesize your answer DIRECTLY from the summaries — do not make additional tool calls.\n\n"
    "SYNTHESIZE rules:\n"
    "- ONLY state facts that are explicitly supported by the retrieved sources.\n"
    "- Every factual claim MUST have an inline citation [Author Year](dataset_url) immediately after the claim.\n"
    "- Do NOT add information from your own training knowledge. If a topic is not covered by\n"
    "  the retrieved sources, say: 'Zu [Thema] liegen keine Quellen in der Datenbank vor.'\n"
    "- Do NOT create separate sections for 'confirmed' vs. 'practical' or 'common' alternatives\n"
    "  — everything you state must come from the sources.\n\n"
    "Keep it fast — max 2-3 tool calls total (find_relevant_groups + literature_search → answer).\n\n"
    ...
    "RESPONSE FORMAT:\n"
    "- Clear, direct answer synthesizing ONLY what the retrieved sources state.\n"
    "- STRICT CITATION RULE: Every factual claim MUST be cited inline as [Author Year](dataset_url)\n"
    "  immediately after the sentence or clause it supports. Example: 'HNBR zeigt gute\n"
    "  Kraftstoffbeständigkeit bis 150 °C [Drobny 2006](https://host/dataset/UUID).'\n"
    "  Use square brackets [] for the label, round brackets () for the URL — standard markdown link syntax.\n"
    "  WRONG: (Author Year)(url) — this is NOT valid markdown.\n"
    "- NO UNSOURCED CLAIMS: Do not add material recommendations, temperature ranges, chemical\n"
    "  properties, or any other technical facts from your own knowledge. If the retrieved sources\n"
    "  do not cover a sub-topic, write: 'Zu [Thema] liegen keine Quellen in der Datenbank vor.'\n"
    "- Do NOT create sections like 'Praxisnahe Alternativen' or 'branchenüblich' that present\n"
...
research_agent_prompt = (
    ...
    "- ONLY state facts that are explicitly supported by the retrieved sources.\n"
    "  Every sentence with a factual claim MUST have an inline citation [Author Year](dataset_url).\n"
    "- Do NOT supplement with your own knowledge. If the sources do not cover a sub-topic,\n"
    "  state: 'Zu [Thema] liegen keine Quellen in der Datenbank vor.'\n"
    ...
    "CITATION FORMAT:\n"
    "- Inline: [Author Year](dataset_url) — immediately after each claim\n"
    "- NO numbered references [1] or [^1^], NO footnotes\n"
    "- STRICT RULE: Every factual claim MUST be cited inline. Example:\n"
    "  'HNBR zeigt gute Kraftstoffbeständigkeit bis 150 °C [Drobny 2006](https://host/dataset/UUID).'\n"
    "  Use square brackets [] for the label, round brackets () for the URL — standard markdown link syntax.\n"
    "  WRONG: (Author Year)(url) — this is NOT valid markdown.\n"
    "- NO UNSOURCED CLAIMS: Do not state material properties, temperature ranges, chemical\n"
    "  resistances, or any technical facts from your own knowledge. ONLY state what the\n"
    "  retrieved sources explicitly say. If a sub-topic is not covered by sources, write:\n"
    ...
    "- Evidence-based only — ZERO assumptions or training-data knowledge as facts\n"
    ...
    for attempt in range(config.MAX_RETRIES_LITERATURE_SEARCH):
        ...
        try:
            ...
            async def _search_and_evaluate():
                ...
                if not hits:
                    return LitSearchResult(results=[], search_str=search_queries)
            ...
            return json.dumps({"error": [f"Literature search timed out after {duration_ms/1000:.0f}s"]})

        except UsageLimitExceeded as e:
            ctx.deps.seen_chunk_ids = seen_before
            log.error(f"literature_search usage limit exceeded on attempt {attempt+1}: {e}, seen_ids rolled back")
            return json.dumps({"error": [f"Token limit exceeded: {str(e)}"]})
            ...
            if attempt == config.MAX_RETRIES_LITERATURE_SEARCH - 1:
                return json.dumps({"error": [f"Model output validation failed: {str(e)}"]})
            continue
            ...
            if attempt == config.MAX_RETRIES_LITERATURE_SEARCH - 1:
                return json.dumps({"error": [f"Literature search failed: {type(e).__name__}: {str(e)[:200]}"]})
            continue

    return json.dumps({"error": ["All literature_search retries exhausted"]})
