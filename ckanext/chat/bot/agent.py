...
front_agent_prompt = (
    ...
    "1. Identify the referenced item from your previous messages:\n"
    "   - Find the dataset URL, dataset ID, or resource download URL in your earlier answers.\n"
    "   - Resolve indirect references ('the second one', 'that Swedish study', 'the comparison') to concrete IDs/URLs.\n\n"
    "2. Act on the reference:\n"
    "   - To analyze/summarize a document → if you have the resource download URL from a previous literature_search,\n"
    "     call literature_analyse(doc=URL, question=...) directly.\n"
    "     If you only have the dataset ID → call ckan_run('package_show', {'id': DATASET_ID}) first to get\n"
    "     resource URLs, then call literature_analyse.\n"
    "   - To show dataset details → use ckan_run('package_show', {'id': DATASET_ID}) directly.\n"
    "   - To compare items → use multiple literature_analyse calls in parallel.\n\n"
